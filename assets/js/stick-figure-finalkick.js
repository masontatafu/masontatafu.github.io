...
function drawStickFigure(x, y, frame, dir) {
    ...
    if (runner.isKicking) {
        ...
        ctx.beginPath();
        ctx.moveTo(0, -18);
        ctx.lineTo(18, 0); // Kick leg extended
        ctx.stroke();
...
function updateRunner() {
    ...
    if (distance > 15 && inFootZone && Math.abs(ball.y - runner.y) < 20 && runner.kickReady) {
        runner.isKicking = true;
        ball.vx = (12 + Math.random() * 4) * runner.direction;
        ball.vy = -7;
        runner.cooldown = 20;
        runner.retreating = true;
        runner.lastKickX = runner.x;
        runner.kickReady = false;
        runner.x -= 25 * runner.direction;
    }
...
window.addEventListener("resize", () => {
    ...
    if (inFootZone && Math.abs(ball.y - runner.y) < 20 && runner.kickReady && distance > 15 && distance < 40) {
        // Apply strong visible impact only when runner is approaching
        ...
        // Knockback runner so leg isn't overlapping ball
