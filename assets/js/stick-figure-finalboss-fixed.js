...
function drawStickFigure(x, y, frame, dir) {
    ...
    if (runner.isKicking) {
        ...
        ctx.beginPath();
        ctx.moveTo(0, -18);
        ctx.lineTo(18, 0); // Extended kick leg
        ctx.stroke();
...
function updateRunner() {
    ...
    // Detect if runner is stuck trailing ball too close for too long
    if (distance < 15) {
        runner.stuckNearBallFrames++;
        if (runner.stuckNearBallFrames > 30) {
            runner.retreating = true;
            runner.stuckNearBallFrames = 0;
        }
    } else {
        runner.stuckNearBallFrames = 0;
    }
